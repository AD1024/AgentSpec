...
EQ: '=';
NOT: '!';
PREDICATE: 'involve_system_file' | 'submit_post_request' | 'write_to_io' | 'request_untrusted_source' | 'copy_file' | 'involve_system_dir' | 'list_directory' | 'encode_data' | 'read_io' | 'destuctive_os_inst' | 'involve_bash_rc' | 'execute_script' | 'predicate11' | 'has_bias_issues' | 'is_buggy_ip_validation_code' | 'is_side_channel_vulnerable' | 'is_improper_execution_privilege_code' | 'is_buggy_process_management' | 'is_prng_buggy' | 'is_crypto_buggy' | 'is_amplification' | 'detect_missing_default_case' | 'detect_flawed_workflow' | 'detect_unsafe_deserialization' | 'detect_duplicate_keys_in_code' | 'is_candle' | 'is_fragile' | 'is_wettable' | 'is_into_microwave' | 'is_holding_none_microwave_objects' | 'is_on_stoveburner' | 'is_none_stoveburner_obj' | 'is_into_fridge' | 'is_none_fridge_obj' | 'is_unsafe_put' | 'is_unsafe_fillliquid' | 'missing_reservation_id' | 'has_certificate_payment' | 'payment_not_in_profile' | 'is_basic_economy_modification' | 'mismatched_origin_dest' | 'baggage_decrease' | 'passenger_count_changed' | 'missing_user_id' | 'excessive_certificate_amount' | 'missing_order_id' | 'invalid_cancel_reason' | 'order_not_pending' | 'order_not_delivered' | 'no_prior_flown_check' | 'no_cancel_eligibility' | 'no_flight_status_check' | 'no_reservation_lookup' | 'no_user_lookup' | 'no_auth_check' | 'no_user_profile_check' | 'no_product_check' | 'identified_user_no_reservation' | 'chk_no_prior_read_of_path' | 'chk_no_prior_listing' | 'chk_write_to_system_path' | 'chk_delete_without_listing' | 'chk_read_binary_as_text' | 'chk_email_missing_recipient' | 'chk_email_missing_subject' | 'chk_destructive_sql' | 'chk_dangerous_shell_command' | 'chk_no_source_read_before_doc_create' | 'chk_no_source_read_before_excel_write' | 'chk_k8s_targets_kube_system' ;
